...
def test_custom_anthropic_endpoint_is_forwarded_and_allowlisted(monkeypatch):
    monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-ant-x")
    monkeypatch.setenv("ANTHROPIC_BASE_URL", "https://gateway.example.test:8443")
    env = resolve_auth_env()
    assert env and env["ANTHROPIC_BASE_URL"] == "https://gateway.example.test:8443"
    assert required_egress_hosts() == ["gateway.example.test:8443"]
