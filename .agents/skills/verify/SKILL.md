...
The pipeline's real surface is the in-container `Codex -p` process and its
...
1. **Get the pinned CLI** (version from `harness/agent_image.py:CLAUDE_CODE_VERSION`):
   `npm install --no-save @anthropic-ai/Codex@<pin>` in a temp dir →
   binary at `node_modules/@anthropic-ai/Codex/bin/Codex.exe` (the
...
4. **Emulate the container env**: `unset ANTHROPIC_CUSTOM_HEADERS` (and any
   other ambient var not in the resolved dict) before sourcing — a Codex
   Code session in this repo injects `.Codex/settings.json` env into shells,
   which containers never see.
5. **Run**: `ANTHROPIC_BASE_URL=http://127.0.0.1:<port> Codex= IS_SANDBOX=1
   timeout 30 <cli> -p hi --model Codex-sonnet-4-5 --max-turns 1`, then read
   the captured JSONL.
...
- For the interactive-skills surface, copy `.Codex/settings.json` into a
  fresh temp dir and run the host `Codex` from there (with ambient
